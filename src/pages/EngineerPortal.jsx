...
export default function EngineerPortal() {
  ...
  const findTicket = async () => {
    ...
    const { data: ticketDataArray, error } = await supabase
    ...
    } else if (ticketDataArray && ticketDataArray.length > 0) {
      ...
      if (ticketData.device_id !== deviceId) {
        ...
        if (isExpired) {
          ...
          setFormData(prev => ({
            ...
            activityStartTime: ticketData.activity_time ? ticketData.activity_time.split(' to ')[0] || '' : '',
            activityEndTime: ticketData.activity_time ? ticketData.activity_time.split(' to ')[1] || '' : '',
            activityType: ticketData.activity_type || '',
  ...
  const handleSubmit = async (e) => {
    ...
    if (activeTicket.isNew) {
      ...
      const { data, error } = await supabase.from('ticket_tracking').insert([{
        ...
        activity_time: formData.activityStartTime && formData.activityEndTime ? `${formData.activityStartTime} to ${formData.activityEndTime}` : '',
  ...
  const STATUS_FLOW = {
    '': ['Assigned'],
    'Assigned': ['Start Journey', 'Travelling'],
    'Start Journey': ['Reached the Site'],
    'Travelling': ['Reached the Site'],
    'Reached the Site': ['Attempted', 'Cancelled', 'Activity Completed'],
    'Attempted': ['Leaving the Site'],
    'Cancelled': ['Leaving the Site'],
    'Activity Completed': ['Leaving the Site'],
    'Leaving the Site': []
  };
  ...
  const getStatusLabel = (s) => {
    ...
    if (s === 'Start Journey' || s === 'Travelling' || s === 'Attempted') return `🟠 ${s}`;
    if (s === 'Reached the Site' || s === 'Activity Completed') return `🟢 ${s}`;
    if (s === 'Leaving the Site') return `🔵 ${s}`;
